## Code Credit
The RAFT dataset generation (raft.py) and evaluation (eval.py) scripts, along with this accompanying README, are sourced from the original RAFT paper's GitHub repository. These components were used to generate the RAFT dataset (output.jsonl) and the baseline evaluation dataset (baseline_eval.jsonl) rescpectively for this project.
...
C, these can be found in the AI Studio. 
```bash 
